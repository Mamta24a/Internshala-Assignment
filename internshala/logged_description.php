...
<input type="text" value="<?php echo "$row[post_id]";?>" name="post_id" hidden>
            ...
            <?php
			 ...
			 $query1=$conn->query("SELECT COUNT(post_id) AS total1 FROM app_recieved WHERE  email='".$_SESSION['userSession2']."' ");
			 $query2=$conn->query("SELECT COUNT(email) AS total2 FROM app_recieved WHERE   post_id=$id ");
			 $b1=$query1->fetch_array(); 
			 $b2=$query2->fetch_array(); 
			 $b1['total1'];
			 $b2['total2'];
			if (!$b1['total1']>0 || !$b2['total2']>0){
			...
			{  ?>
		      <div class='alert alert-danger'>
                <h4><span class='glyphicon glyphicon-info-sign'></span> &nbsp; You have already applied for this internship!</h4>
              </div></form>
		<form action="loggedin_users.php">
		<button style="padding:1% 8%" type="submit" name="apply" class="btn btn-primary" >Go to Internship Search</button>
	    </form>
			<?php }	?>
